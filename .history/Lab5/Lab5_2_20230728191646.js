...
const closeAddEl = document.querySelector(".icon-close-add");
    ...
    formAdd.querySelectorAll("input").forEach((element) => {
      if (element.name == "img") {
        ...
        if (files) {
          ...
          fr.onload = function () {
            console.log("okok", fr.result);
            data1 = { ...data1, img: fr.result + "" };
            console.log(data1);
            data.unshift(data1);
            renderProducts();
        ...
        data1 = { ...data1, [element.name]: element.value };
      }
    });
  };
}
closeAddEl.onclick = (e) => {
  modalAdd.classList.add("hidden");
};
...
const v = document.querySelector(".btn");
v.onclick = () => {
  a++;
  console.log(a);
};
