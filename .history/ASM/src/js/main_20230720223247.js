...
const languageEls = document.querySelectorAll(".lg");

const changeLanguage = () => {
  if (languageEls)
    languageEls.forEach((element) => {
      const nameLg = element.classList[1].split("-")[1];
      if (nameLg) {
        element.textContent = language[LANGUAGE][nameLg];
      }
    });
};

changeLanguage();
const lguageEl = document.querySelectorAll(".header__notify-name");
const lguageTittle = document.querySelectorAll(".lg-vietnam");
if (lguageEl) {
  lguageEl.forEach((element) => {
    element.addEventListener("click", () => {
      if (element.textContent == "Tiếng Việt") {
        LANGUAGE = "vi";
      } else {
        LANGUAGE = "en";
      }
  ...
  <div >
    ...
    <span class="h-name"
      ...
      <div class="header-logout">
        ...
        <div class="lg lg-logout logout">Đăng xuất</div>
      </div>
    </span>
  </div>
  `;
...
if (store?.state?.cart.length <= 0) {
  ...
  if (showCart) {
    let htmlCartList = store?.state?.cart.map((item) => {
      const productInfo = dataProducts.find((pro) => pro.id == item.id);
      return `
      <div class="s_has-cart-item">
        <div class="s_has-cart-item-img">
          <img src=${productInfo?.img[0]} alt="" />
        </div>
...
const searchEl = document.querySelector(".s_input");
const listSearchEl = document.querySelector(".s-find-list");
...
if (searchEl) {
  searchEl.oninput = (e) => {
    ...
    const arr = dataProducts.filter((item) =>
      item.name.toLocaleUpperCase().includes(value)
    );
    let newArr = [];
    if (arr.length > 5) {
      newArr = arr.slice(arr.length - 5, arr.length);
    } else {
      newArr = [...arr];
    }
    if (listSearchEl) {
      let htmlListSearch = newArr.map((item) => {
        return `
        <div class="s-find-item">
          <img src=${item.img[0]} alt="" />
          <a href="./src/page/detail.html?id=${item.id}">${item.name}</a>
        </div>
        `;
      });
      listSearchEl.innerHTML = htmlListSearch.join("");
    }
  };
}
