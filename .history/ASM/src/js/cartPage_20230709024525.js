...
if (listProduct) {
  ...
  //   handle event
  const groupEl = document.querySelectorAll(".b-group-body");
  if (groupEl) {
    groupEl.forEach((element) => {
      // add and remove count of product
      const priceEl = element.querySelector(".price-one-product");
      const removeEl = element.querySelector(".price-one-product");
      const addEl = element.querySelector(".price-one-product");
      const countEl = element.querySelector(".count-numbeer");
      const totalOneTypeEl = element.querySelector(".price-one-product");
    });
  }
}
